...
var BiometricAuth = (() => {
  ...
  // ⚡ PERFORMANCE FIX (Apr 2026):
  // Previously polled every second with setInterval to detect when the
  // auth overlay opens. That's a wakeup-per-second forever, bad for
  // mobile battery. Switched to MutationObserver which only fires when
  // the overlay's class actually changes.
  let _bioInitDone = false;
  function watchAuthOverlay() {
    const overlay = document.getElementById('authOverlay');
    if (!overlay) {
      // Element not yet in DOM — try again after DOM is ready, once.
      if (document.readyState !== 'complete') {
        document.addEventListener('DOMContentLoaded', watchAuthOverlay, { once: true });
      }
      return;
    }
    // If it's already open right now, init immediately.
    if (overlay.classList.contains('open') && !_bioInitDone) {
      initLoginButton();
      _bioInitDone = true;
    }
    ...
    const observer = new MutationObserver(() => {
      if (overlay.classList.contains('open')) {
        ...
        // Reset so re-opening triggers init again (e.g. user logs out).
        _bioInitDone = false;
      }
    });
