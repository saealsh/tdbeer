...
(function bootstrap() {
  ...
  function startModules() {
    // ═══ Performance أولاً (يدير الباقي) ═══
    safeRun(() => window.Performance?.init?.(), 'Bootstrap.Performance');
    ...
    const modules = [
      ...
      ['SmartSearch',       () => window.SmartSearch?.init?.()],
      ['Heatmap',           () => window.Heatmap?.init?.()],
      ['PushNotifs',        () => window.PushNotifs?.init?.()],
      ['UIPolish',          () => window.UIPolish?.init?.()],
      ['Diagnostics',       () => window.Diagnostics?.init?.()]
    ];
