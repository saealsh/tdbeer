...
var ChatNotifications = (() => {
  ...
  function createAudioContext() {
    ...
    try {
      ...
      window.Logger?.warn?.('[Notifications] AudioContext not supported');
      return null;
    }
  }
  ...
  function playMessageSound() {
    ...
    try {
      ...
      window.Logger?.warn?.('[Notifications] Sound error:', e.message);
    }
  }
    ...
    showChatToast({
      ...
      onOpen: () => {
        try {
          ...
          if (social?.openChat) social.openChat(friend);
          hidePersistentBar();
        } catch (e) { window.Logger?.warn?.('Open chat error:', e.message); }
      }
    });
  }
