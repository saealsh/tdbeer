...
var Logger = (() => {
  ...
  function push(level, ctx, msg, meta) {
    ...
    // Production detection - تطبع فقط لو في dev mode
    const isProd = window.__PROD__ === true 
                || (location.hostname !== 'localhost' 
                    && location.hostname !== '127.0.0.1'
                    && !location.hostname.startsWith('192.168.')
                    && !location.hostname.includes('.local'));
    
    // في dev mode: طبع كل المستويات
    // في production: فقط warn/error
    const shouldLog = !isProd || level === 'error' || level === 'warn';
