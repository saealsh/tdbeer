...
## 🆕 إصلاح #13 — الشريط السفلي مازال مشوّه (المحاولة الثالثة)

**الملفات:** `css/main.css` + `css/features.css`

**المشكلة (من الصورة الجديدة):**
رغم الإصلاحات السابقة، الشريط السفلي مازال مشوّه:
- زر "+" الأخضر بارز خارج إطار الشريط (الشريط مقطوع من تحته)
- الأزرار غير متوازنة عمودياً
- الشريط يبدو شفاف ومن غير حدود واضحة

**سبب الإخفاق في المحاولات السابقة:**
- المحاولة #1 استخدمت `margin-top: -22px` → كسرت Grid layout
- المحاولة #2 استخدمت `transform: translateY(-14px)` + `align-items: end` → الـ FAB يبرز للأعلى لكن الشريط لا يوفر مساحة لاستيعاب البروز، فالزر يخرج خارج الإطار

**الإصلاح الجديد (نهج مختلف):**
**التخلي عن فكرة بروز الـ FAB للأعلى تماماً.** بدلاً من ذلك:
- الـ FAB يبقى داخل الشريط بنفس الارتفاع (48×48 px)
- يتميز بصرياً بـ: حجم أكبر قليلاً + شكل دائري + لون أخضر فاقع + ظل واضح + إطار من لون الخلفية
- `align-items: center` على Grid → كل الأزرار تتمحور بنفس المستوى
- `min-height: 64px` للشريط → ارتفاع ثابت يضمن استقرار التخطيط
- `padding: 8px` بدل padding-top فقط → توزيع متساوي
...
.main-nav-inner {
  display: grid;
  grid-template-columns: repeat(5, 1fr);
  align-items: center;       /* تغيّر من end إلى center */
  padding: 8px var(--s-1);
  min-height: 64px;
}
.tab-btn-fab {
  width: 48px !important;     /* أصغر من قبل */
...
**النتيجة المتوقعة:**
- 5 أزرار في صف واحد، نفس الارتفاع
- الـ FAB يتميز بسهولة بـ اللون والشكل دون الحاجة لبروز يكسر التخطيط
- الشريط له خلفية وحدود واضحة
- يعمل بشكل متطابق على الموبايل والديسكتوب

**Service Worker:** 5.1.0 → 5.1.1
